...
_distutils_set_globals() {
	...
	if [[ ${DISTUTILS_USE_PEP517} ]]; then
		...
		# pip is used to install the wheel (TODO: replace with installer
		# once it finally gets a CLI)
...
_distutils-r1_create_setup_cfg() {
	cat > "${HOME}"/.pydistutils.cfg <<-_EOF_ || die
		...
		# speed up temporary wheel builds
		[bdist_wheel]
		skip_build = True
		compression = stored
...
distutils-r1_python_compile() {
	...
	if [[ ${DISTUTILS_USE_PEP517} ]]; then
		...
		local build_backend
		if [[ ! -f pyproject.toml && ${DISTUTILS_USE_PEP517} == setuptools &&
			-f setup.py ]]
		then
		...
		pip install --root "${root}" --no-deps --disable-pip-version-check \
